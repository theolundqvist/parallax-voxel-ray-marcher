...
constexpr float scale_lengths = 100.0f; // The scene is expressed in centimetres
                                        // rather than metres, hence the x100.
...
enum class ElapsedTimeQuery : uint32_t {
  ...
  Light0Accumulation =
      ShadowMap0Generation + static_cast<uint32_t>(constant::lights_nb),
...
void fillGBufferShaderLocations(GLuint gbuffer_shader,
...
void fillShadowmapShaderLocations(GLuint shadowmap_shader,
...
void fillAccumulateLightsShaderLocations(
    GLuint accumulate_lights_shader,
...
edan35::Assignment2::Assignment2(WindowManager &windowManager)
  ...
  window = mWindowManager.CreateGLFWWindow("EDAN35: Assignment 2", window_datum,
                                           config::msaa_rate);
  if (window == nullptr) {
...
edan35::Assignment2::~Assignment2() { bonobo::deinit(); }
...
void edan35::Assignment2::run() {
  ...
  for (auto const &geometry : sponza_geometry) {
    if (!strstr(geometry.name.c_str(), search.c_str())) {
      temp.push_back(geometry);
    } else {
      std::cout << "removed " << geometry.name << std::endl;
    }
  }
  sponza_geometry = temp;
  ...
  mCamera.mWorld.SetTranslate(glm::vec3(0.0f, 1.0f, 1.8f) *
                              constant::scale_lengths);
  mCamera.mMouseSensitivity = glm::vec2(0.003f);
  mCamera.mMovementSpeed =
      glm::vec3(3.0f) * constant::scale_lengths; // 3 m/s => 10.8 km/h.
  ...
  // Look further down in this file to see the implementation of those
  // functions.
  //
  Textures const textures =
      createTextures(framebuffer_width, framebuffer_height);
  ...
  fillGBufferShaderLocations(fill_gbuffer_shader,
                             fill_gbuffer_shader_locations);
  ...
  fillShadowmapShaderLocations(fill_shadowmap_shader,
                               fill_shadowmap_shader_locations);
  ...
  fillAccumulateLightsShaderLocations(accumulate_lights_shader,
                                      accumulate_light_shader_locations);
  ...
  std::array<ViewProjTransforms, constant::lights_nb>
      light_view_proj_transforms;

  const GLuint debug_texture_id = bonobo::getDebugTextureID();

  auto const bind_texture_with_sampler =
      [](GLenum target, unsigned int slot, GLuint program,
         std::string const &name, GLuint texture, GLuint sampler) {
  ...
  for (size_t i = 0; i < static_cast<size_t>(lights_nb); ++i) {
    if (i == 0) {
      ...
      lightTransforms[i].SetTranslate(glm::vec3(10.0f, 25.00f, 7.0f) *
                                      constant::scale_lengths);
      lightTransforms[i].LookAt(glm::vec3(0.0f, 10.0f, 0.0f));
      lightColors[i] = glm::vec3(1.0f, 1.0f, 0.9f);
    } else {

      lightTransforms[i].SetTranslate(glm::vec3(0.0f, 1.25f, 0.0f) *
                                      constant::scale_lengths);
      lightColors[i] = glm::vec3(0.5f + 0.5f * (static_cast<float>(rand()) /
                                                static_cast<float>(RAND_MAX)),
                                 0.5f + 0.5f * (static_cast<float>(rand()) /
                                                static_cast<float>(RAND_MAX)),
                                 0.5f + 0.5f * (static_cast<float>(rand()) /
                                                static_cast<float>(RAND_MAX)));
    }
  ...
  float const lightProjectionFarPlane = 40.0f * constant::scale_lengths;
  auto lightProjection =
      glm::perspective(0.5f * glm::pi<float>(),
                       static_cast<float>(constant::shadowmap_res_x) /
                           static_cast<float>(constant::shadowmap_res_y),
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (!are_lights_paused)
      seconds_nb +=
          std::chrono::duration<decltype(seconds_nb)>(deltaTimeUs).count();
    ...
    camera_view_proj_transforms.view_projection =
        mCamera.GetWorldToClipMatrix();
    camera_view_proj_transforms.view_projection_inverse =
        mCamera.GetClipToWorldMatrix();
    ...
    if (!first_frame && show_gui && copy_elapsed_times) {
      ...
      for (GLuint i = 0; i < pass_elapsed_times.size(); ++i) {
        glGetQueryObjectui64v(elapsed_time_queries[i], GL_QUERY_RESULT,
                              pass_elapsed_times.data() + i);
      }
    }
    ...
    for (size_t i = 0; i < static_cast<size_t>(lights_nb); ++i) {
      ...
      if (i != 0) {
        ...
        lightTransform.SetRotate(
            glm::two_pi<float>() * static_cast<float>(i) /
                    static_cast<float>(constant::lights_nb) +
                0.1f * seconds_nb,
      ...
      auto const light_world_matrix =
          glm::inverse(light_view_matrix) * coneScaleTransform.GetMatrix();
      auto const light_world_to_clip_matrix =
          lightProjection * light_view_matrix;

      light_view_proj_transforms[i].view_projection =
          light_world_to_clip_matrix;
      light_view_proj_transforms[i].view_projection_inverse =
          glm::inverse(light_world_to_clip_matrix);
    }
    ...
    glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(camera_view_proj_transforms),
                    &camera_view_proj_transforms);
    glBindBuffer(GL_UNIFORM_BUFFER, ubos[toU(UBO::LightViewProjTransforms)]);
    glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(light_view_proj_transforms),
                    light_view_proj_transforms.data());
    ...
    if (!shader_reload_failed) {
      ...
      glBeginQuery(
          GL_TIME_ELAPSED,
          elapsed_time_queries[toU(ElapsedTimeQuery::GbufferGeneration)]);
      ...
      for (std::size_t i = 0; i < sponza_geometry.size(); ++i) {
        ...
        glUniformMatrix4fv(fill_gbuffer_shader_locations.vertex_model_to_world,
                           1, GL_FALSE, glm::value_ptr(vertex_model_to_world));
        glUniformMatrix4fv(fill_gbuffer_shader_locations.normal_model_to_world,
        ...
        glUniform1i(fill_gbuffer_shader_locations.has_diffuse_texture,
                    texture_data.diffuse_texture_id != 0u ? 1 : 0);
        glBindSampler(0u, texture_data.diffuse_texture_id != 0u
                              ? mipmap_sampler
                              : default_sampler);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, texture_data.diffuse_texture_id != 0u
                                         ? texture_data.diffuse_texture_id
                                         : debug_texture_id);

        glUniform1i(fill_gbuffer_shader_locations.has_specular_texture,
                    texture_data.specular_texture_id != 0u ? 1 : 0);
        glBindSampler(1u, texture_data.specular_texture_id != 0u
                              ? mipmap_sampler
                              : default_sampler);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, texture_data.specular_texture_id != 0u
                                         ? texture_data.specular_texture_id
                                         : debug_texture_id);

        glUniform1i(fill_gbuffer_shader_locations.has_normals_texture,
                    texture_data.normals_texture_id != 0u ? 1 : 0);
        glBindSampler(2u, texture_data.normals_texture_id != 0u
                              ? mipmap_sampler
                              : default_sampler);
        glActiveTexture(GL_TEXTURE2);
        glBindTexture(GL_TEXTURE_2D, texture_data.normals_texture_id != 0u
                                         ? texture_data.normals_texture_id
                                         : debug_texture_id);

        glUniform1i(fill_gbuffer_shader_locations.has_opacity_texture,
                    texture_data.opacity_texture_id != 0u ? 1 : 0);
        glBindSampler(3u, texture_data.opacity_texture_id != 0u
                              ? mipmap_sampler
                              : default_sampler);
        glActiveTexture(GL_TEXTURE3);
        glBindTexture(GL_TEXTURE_2D, texture_data.opacity_texture_id != 0u
                                         ? texture_data.opacity_texture_id
                                         : debug_texture_id);

        glBindVertexArray(geometry.vao);
        if (geometry.ibo != 0u)
          glDrawElements(geometry.drawing_mode, geometry.indices_nb,
                         GL_UNSIGNED_INT,
      ...
      for (size_t i = 0; i < static_cast<size_t>(lights_nb); ++i) {
        ...
        //
        utils::opengl::debug::beginDebugGroup("Create shadow map " +
                                              std::to_string(i));
        glBeginQuery(
            GL_TIME_ELAPSED,
            elapsed_time_queries[toU(ElapsedTimeQuery::ShadowMap0Generation) +
                                 i]);
        ...
        glUniform1i(fill_shadowmap_shader_locations.light_index,
                    static_cast<int>(i));
        ...
        for (std::size_t i = 0; i < sponza_geometry.size(); ++i) {
          ...
          glUniform1i(fill_shadowmap_shader_locations.has_opacity_texture,
                      texture_data.opacity_texture_id != 0u ? 1 : 0);
          glBindSampler(0u, texture_data.opacity_texture_id != 0u
                                ? samplers[toU(Sampler::Mipmaps)]
                                : samplers[toU(Sampler::Nearest)]);
          glActiveTexture(GL_TEXTURE0);
          glBindTexture(GL_TEXTURE_2D, texture_data.opacity_texture_id != 0u
                                           ? texture_data.opacity_texture_id
                                           : debug_texture_id);
        ...
        utils::opengl::debug::beginDebugGroup("Accumulate light " +
                                              std::to_string(i));
        glBeginQuery(
            GL_TIME_ELAPSED,
            elapsed_time_queries[toU(ElapsedTimeQuery::Light0Accumulation) +
                                 i]);
        ...
        glUniform1i(accumulate_light_shader_locations.light_index,
                    static_cast<int>(i));
        glUniformMatrix4fv(
            accumulate_light_shader_locations.vertex_model_to_world, 1,
            GL_FALSE, glm::value_ptr(light_world_matrix));
        ...
        glUniform3fv(accumulate_light_shader_locations.light_color, 1,
                     glm::value_ptr(lightColors[i]));
        glUniform3fv(accumulate_light_shader_locations.light_position, 1,
                     glm::value_ptr(lightTransform.GetTranslation()));
        glUniform3fv(accumulate_light_shader_locations.light_direction, 1,
                     glm::value_ptr(lightTransform.GetFront()));
        glUniform1f(accumulate_light_shader_locations.light_intensity,
                    constant::light_intensity);
        glUniform1f(accumulate_light_shader_locations.light_angle_falloff,
        ...
        glBindTexture(GL_TEXTURE_2D,
                      textures[toU(Texture::GBufferWorldSpaceNormal)]);
      ...
      //
      // Pass 3: Compute final image using both the g-buffer and  the light
      // accumulation buffer
      //
      utils::opengl::debug::beginDebugGroup("Resolve");
      glBeginQuery(GL_TIME_ELAPSED,
                   elapsed_time_queries[toU(ElapsedTimeQuery::Resolve)]);
      ...
      bind_texture_with_sampler(GL_TEXTURE_2D, 0, resolve_deferred_shader,
                                "diffuse_texture",
                                textures[toU(Texture::GBufferDiffuse)],
                                samplers[toU(Sampler::Nearest)]);
      bind_texture_with_sampler(GL_TEXTURE_2D, 1, resolve_deferred_shader,
                                "specular_texture",
                                textures[toU(Texture::GBufferSpecular)],
                                samplers[toU(Sampler::Nearest)]);
      bind_texture_with_sampler(
          GL_TEXTURE_2D, 2, resolve_deferred_shader, "light_d_texture",
          textures[toU(Texture::LightDiffuseContribution)],
          samplers[toU(Sampler::Nearest)]);
      bind_texture_with_sampler(
          GL_TEXTURE_2D, 3, resolve_deferred_shader, "light_s_texture",
          textures[toU(Texture::LightSpecularContribution)],
          samplers[toU(Sampler::Nearest)]);
    ...
    //
    glBeginQuery(GL_TIME_ELAPSED,
                 elapsed_time_queries[toU(ElapsedTimeQuery::ConeWireframe)]);
    if (show_cone_wireframe) {
      ...
      for (size_t i = 0; i < lights_nb; ++i) {
        cone.render(view_projection,
                    lightTransforms[i].GetMatrix() *
                        lightOffsetTransform.GetMatrix() *
                        coneScaleTransform.GetMatrix(),
    ...
    glBeginQuery(GL_TIME_ELAPSED,
                 elapsed_time_queries[toU(ElapsedTimeQuery::GUI)]);
    ...
    //
    // Output content of the g-buffer as well as of the shadowmap, for debugging
    // purposes
    //
    if (show_textures) {
      bonobo::displayTexture({-0.95f, -0.95f}, {-0.55f, -0.55f},
                             textures[toU(Texture::GBufferDiffuse)],
                             samplers[toU(Sampler::Linear)], {0, 1, 2, -1},
                             glm::uvec2(framebuffer_width, framebuffer_height));
      bonobo::displayTexture({-0.45f, -0.95f}, {-0.05f, -0.55f},
                             textures[toU(Texture::GBufferSpecular)],
                             samplers[toU(Sampler::Linear)], {0, 1, 2, -1},
                             glm::uvec2(framebuffer_width, framebuffer_height));
      bonobo::displayTexture({0.05f, -0.95f}, {0.45f, -0.55f},
                             textures[toU(Texture::GBufferWorldSpaceNormal)],
                             samplers[toU(Sampler::Linear)], {0, 1, 2, -1},
                             glm::uvec2(framebuffer_width, framebuffer_height));
      bonobo::displayTexture({0.55f, -0.95f}, {0.95f, -0.55f},
                             textures[toU(Texture::DepthBuffer)],
                             samplers[toU(Sampler::Linear)], {0, 0, 0, -1},
                             glm::uvec2(framebuffer_width, framebuffer_height),
                             true, mCamera.mNear, mCamera.mFar);
      bonobo::displayTexture(
          {-0.95f, 0.55f}, {-0.55f, 0.95f}, textures[toU(Texture::ShadowMap)],
          samplers[toU(Sampler::Linear)], {0, 0, 0, -1},
          glm::uvec2(framebuffer_width, framebuffer_height), true,
          lightProjectionNearPlane, lightProjectionFarPlane);
      bonobo::displayTexture({-0.45f, 0.55f}, {-0.05f, 0.95f},
                             textures[toU(Texture::LightDiffuseContribution)],
                             samplers[toU(Sampler::Linear)], {0, 1, 2, -1},
                             glm::uvec2(framebuffer_width, framebuffer_height));
      bonobo::displayTexture({0.05f, 0.55f}, {0.45f, 0.95f},
                             textures[toU(Texture::LightSpecularContribution)],
                             samplers[toU(Sampler::Linear)], {0, 1, 2, -1},
                             glm::uvec2(framebuffer_width, framebuffer_height));
    }
    ...
    if (opened) {
      ...
      if (ImGui::BeginTable("Pass durations", 2,
        ...
        ImGui::Text(
            "%.3f",
            pass_elapsed_times[toU(ElapsedTimeQuery::GbufferGeneration)] /
                1000000.0f);
        ...
        for (std::size_t i = 0; i < lights_nb; ++i) {
          ...
          ImGui::Text(
              "%.3f",
              pass_elapsed_times[toU(ElapsedTimeQuery::ShadowMap0Generation) +
                                 i] /
                  1000000.0f);
          ...
          ImGui::Text(
              "%.3f",
              pass_elapsed_times[toU(ElapsedTimeQuery::Light0Accumulation) +
                                 i] /
                  1000000.0f);
        }
        ...
        ImGui::Text("Resolve");
        ImGui::TableNextColumn();
        ImGui::Text("%.3f", pass_elapsed_times[toU(ElapsedTimeQuery::Resolve)] /
                                1000000.0f);
        ...
        ImGui::TableNextColumn();
        ImGui::Text("%.3f",
                    pass_elapsed_times[toU(ElapsedTimeQuery::ConeWireframe)] /
                        1000000.0f);

        ImGui::TableNextColumn();
        ImGui::Text("GUI");
        ImGui::TableNextColumn();
        ImGui::Text("%.3f", pass_elapsed_times[toU(ElapsedTimeQuery::GUI)] /
                                1000000.0f);
        ...
        ImGui::Text(
            "%.3f",
            pass_elapsed_times[toU(ElapsedTimeQuery::CopyToFramebuffer)] /
                1000000.0f);
    ...
    if (opened) {
      ...
      ImGui::SliderInt("Number of lights", &lights_nb, 1,
                       static_cast<int>(constant::lights_nb));
    ...
    glBeginQuery(
        GL_TIME_ELAPSED,
        elapsed_time_queries[toU(ElapsedTimeQuery::CopyToFramebuffer)]);

    // FBO::Resolve has already been bound to GL_READ_FRAMEBUFFER before
    // rendering the first frame, as no other frame buffer gets bound to it.
  ...
  glDeleteQueries(static_cast<GLsizei>(elapsed_time_queries.size()),
                  elapsed_time_queries.data());
...
Textures createTextures(GLsizei framebuffer_width, GLsizei framebuffer_height) {
  ...
  glBindTexture(GL_TEXTURE_2D, textures[toU(Texture::DepthBuffer)]);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH24_STENCIL8, framebuffer_width,
               framebuffer_height, 0, GL_DEPTH_COMPONENT, GL_FLOAT, nullptr);
  utils::opengl::debug::nameObject(
      GL_TEXTURE, textures[toU(Texture::DepthBuffer)], "Depth buffer");

  glBindTexture(GL_TEXTURE_2D, textures[toU(Texture::ShadowMap)]);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32F,
               constant::shadowmap_res_x, constant::shadowmap_res_y, 0,
               GL_DEPTH_COMPONENT, GL_FLOAT, nullptr);
  utils::opengl::debug::nameObject(
      GL_TEXTURE, textures[toU(Texture::ShadowMap)], "Shadow map");
  ...
  utils::opengl::debug::nameObject(
      GL_TEXTURE, textures[toU(Texture::GBufferDiffuse)], "GBuffer diffuse");

  glBindTexture(GL_TEXTURE_2D, textures[toU(Texture::GBufferSpecular)]);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, framebuffer_width, framebuffer_height,
               0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
  utils::opengl::debug::nameObject(
      GL_TEXTURE, textures[toU(Texture::GBufferSpecular)], "GBuffer specular");

  glBindTexture(GL_TEXTURE_2D, textures[toU(Texture::GBufferWorldSpaceNormal)]);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, framebuffer_width, framebuffer_height,
               0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
  utils::opengl::debug::nameObject(
      GL_TEXTURE, textures[toU(Texture::GBufferWorldSpaceNormal)],
      "GBuffer normals");
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, framebuffer_width, framebuffer_height,
               0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
  utils::opengl::debug::nameObject(
      GL_TEXTURE, textures[toU(Texture::LightDiffuseContribution)],
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, framebuffer_width, framebuffer_height,
               0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
  utils::opengl::debug::nameObject(
      GL_TEXTURE, textures[toU(Texture::LightSpecularContribution)],
      "Light specular contribution");

  glBindTexture(GL_TEXTURE_2D, textures[toU(Texture::Result)]);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, framebuffer_width, framebuffer_height,
               0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
  utils::opengl::debug::nameObject(GL_TEXTURE, textures[toU(Texture::Result)],
                                   "Final result");
...
Samplers createSamplers() {
  ...
  glSamplerParameteri(samplers[toU(Sampler::Nearest)], GL_TEXTURE_MIN_FILTER,
                      GL_NEAREST);
  glSamplerParameteri(samplers[toU(Sampler::Nearest)], GL_TEXTURE_MAG_FILTER,
                      GL_NEAREST);
  utils::opengl::debug::nameObject(GL_SAMPLER, samplers[toU(Sampler::Nearest)],
                                   "Nearest");

  // For sampling 2-D textures without mipmaps.
  glSamplerParameteri(samplers[toU(Sampler::Linear)], GL_TEXTURE_MIN_FILTER,
                      GL_LINEAR);
  glSamplerParameteri(samplers[toU(Sampler::Linear)], GL_TEXTURE_MAG_FILTER,
                      GL_LINEAR);
  utils::opengl::debug::nameObject(GL_SAMPLER, samplers[toU(Sampler::Linear)],
                                   "Linear");

  // For sampling 2-D textures with mipmaps.
  glSamplerParameteri(samplers[toU(Sampler::Mipmaps)], GL_TEXTURE_MIN_FILTER,
                      GL_LINEAR_MIPMAP_LINEAR);
  glSamplerParameteri(samplers[toU(Sampler::Mipmaps)], GL_TEXTURE_MAG_FILTER,
                      GL_LINEAR);
  utils::opengl::debug::nameObject(GL_SAMPLER, samplers[toU(Sampler::Mipmaps)],
                                   "Mimaps");
...
FBOs createFramebufferObjects(Textures const &textures) {
  auto const validate_fbo = [](std::string const &fbo_name) {
    ...
    LogError("Framebuffer \"%s\" is not complete: check the logs for "
  ...
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                         textures[toU(Texture::GBufferDiffuse)], 0);
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D,
                         textures[toU(Texture::GBufferSpecular)], 0);
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, GL_TEXTURE_2D,
                         textures[toU(Texture::GBufferWorldSpaceNormal)], 0);
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D,
                         textures[toU(Texture::DepthBuffer)], 0);
  glReadBuffer(GL_NONE); // Disable reading back from the colour attachments, as
                         // unnecessary in this assignment.
  // Configure the mapping from fragment shader outputs to colour attachments.
  std::array<GLenum, 3> const gbuffer_draws = {
      GL_COLOR_ATTACHMENT0, // The fragment shader output at location 0 will be
                            // written to colour attachment 0 (i.e. the diffuse
                            // texture).
      GL_COLOR_ATTACHMENT1, // The fragment shader output at location 1 will be
                            // written to colour attachment 1 (i.e. the specular
                            // texture).
      GL_COLOR_ATTACHMENT2  // The fragment shader output at location 2 will be
                            // written to colour attachment 2 (i.e. the normal
                            // texture).
  };
  glDrawBuffers(static_cast<GLsizei>(gbuffer_draws.size()),
                gbuffer_draws.data());
  validate_fbo("GBuffer");
  utils::opengl::debug::nameObject(GL_FRAMEBUFFER, fbos[toU(FBO::GBuffer)],
                                   "GBuffer");

  glBindFramebuffer(GL_FRAMEBUFFER, fbos[toU(FBO::ShadowMap)]);
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D,
                         textures[toU(Texture::ShadowMap)], 0);
  validate_fbo("Shadow map generation");
  utils::opengl::debug::nameObject(GL_FRAMEBUFFER, fbos[toU(FBO::ShadowMap)],
                                   "Shadow map generation");

  glBindFramebuffer(GL_FRAMEBUFFER, fbos[toU(FBO::LightAccumulation)]);
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                         textures[toU(Texture::LightDiffuseContribution)], 0);
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D,
                         textures[toU(Texture::LightSpecularContribution)], 0);
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D,
                         textures[toU(Texture::DepthBuffer)], 0);
  glReadBuffer(GL_NONE); // Disable reading back from the colour attachments, as
                         // unnecessary in this assignment.
  ...
  std::array<GLenum, 2> const light_accumulation_draws = {
      GL_COLOR_ATTACHMENT0, // The fragment shader output at location 0 will be
                            // written to colour attachment 0 (i.e. the light
                            // diffuse contribution texture).
      GL_COLOR_ATTACHMENT1  // The fragment shader output at location 1 will be
                            // written to colour attachment 1 (i.e. the light
                            // specular contribution texture).
  };
  glDrawBuffers(static_cast<GLsizei>(light_accumulation_draws.size()),
                light_accumulation_draws.data());
  validate_fbo("Light accumulation");
  utils::opengl::debug::nameObject(
      GL_FRAMEBUFFER, fbos[toU(FBO::LightAccumulation)], "Light acccumulation");

  glBindFramebuffer(GL_FRAMEBUFFER, fbos[toU(FBO::Resolve)]);
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                         textures[toU(Texture::Result)], 0);
  glReadBuffer(
      GL_COLOR_ATTACHMENT0); // Colour attachment result 0 (i.e. the rendering
                             // result texture) will be blitted to the screen.
  glDrawBuffer(GL_COLOR_ATTACHMENT0); // The fragment shader output at location
                                      // 0 will be written to colour attachment
                                      // 0 (i.e. the rendering result texture).
  validate_fbo("Resolve");
  utils::opengl::debug::nameObject(GL_FRAMEBUFFER, fbos[toU(FBO::Resolve)],
                                   "Resolve");
  ...
  utils::opengl::debug::nameObject(
      GL_FRAMEBUFFER, fbos[toU(FBO::FinalWithDepth)], "Cone wireframe");
...
ElapsedTimeQueries createElapsedTimeQueries() {
  ...
  if (utils::opengl::debug::isSupported()) {
    ...
    utils::opengl::debug::nameObject(
        GL_QUERY, queries[toU(ElapsedTimeQuery::GbufferGeneration)],
        "GBuffer generation");

    for (size_t i = 0; i < constant::lights_nb; ++i) {
      register_query(queries[toU(ElapsedTimeQuery::ShadowMap0Generation) + i]);
      utils::opengl::debug::nameObject(
          GL_QUERY, queries[toU(ElapsedTimeQuery::ShadowMap0Generation) + i],
          "Shadow map " + std::to_string(i) + " generation");

      register_query(queries[toU(ElapsedTimeQuery::Light0Accumulation) + i]);
      utils::opengl::debug::nameObject(
          GL_QUERY, queries[toU(ElapsedTimeQuery::Light0Accumulation) + i],
          "Light" + std::to_string(i) + " accumulation");
    }

    register_query(queries[toU(ElapsedTimeQuery::Resolve)]);
    utils::opengl::debug::nameObject(
        GL_QUERY, queries[toU(ElapsedTimeQuery::Resolve)], "Resolve");

    register_query(queries[toU(ElapsedTimeQuery::ConeWireframe)]);
    utils::opengl::debug::nameObject(
        GL_QUERY, queries[toU(ElapsedTimeQuery::ConeWireframe)],
        "Cone wireframe");

    register_query(queries[toU(ElapsedTimeQuery::GUI)]);
    utils::opengl::debug::nameObject(
        GL_QUERY, queries[toU(ElapsedTimeQuery::GUI)], "GUI");
  }
...
UBOs createUniformBufferObjects() {
  ...
  glBindBuffer(GL_UNIFORM_BUFFER, ubos[toU(UBO::CameraViewProjTransforms)]);
  glBufferData(GL_UNIFORM_BUFFER, sizeof(ViewProjTransforms), nullptr,
               GL_STREAM_DRAW);
  glBindBufferBase(GL_UNIFORM_BUFFER, toU(UBO::CameraViewProjTransforms),
                   ubos[toU(UBO::CameraViewProjTransforms)]);
  utils::opengl::debug::nameObject(GL_BUFFER,
                                   ubos[toU(UBO::CameraViewProjTransforms)],
                                   "Camera view-projection transforms");

  glBindBuffer(GL_UNIFORM_BUFFER, ubos[toU(UBO::LightViewProjTransforms)]);
  glBufferData(GL_UNIFORM_BUFFER,
               constant::lights_nb * sizeof(ViewProjTransforms), nullptr,
               GL_STREAM_DRAW);
  glBindBufferBase(GL_UNIFORM_BUFFER, toU(UBO::LightViewProjTransforms),
                   ubos[toU(UBO::LightViewProjTransforms)]);
  utils::opengl::debug::nameObject(GL_BUFFER,
                                   ubos[toU(UBO::LightViewProjTransforms)],
                                   "Light view-projection transforms");
...
void fillGBufferShaderLocations(GLuint gbuffer_shader,
                                GBufferShaderLocations &locations) {
  locations.ubo_CameraViewProjTransforms =
      glGetUniformBlockIndex(gbuffer_shader, "CameraViewProjTransforms");
  locations.vertex_model_to_world =
      glGetUniformLocation(gbuffer_shader, "vertex_model_to_world");
  locations.normal_model_to_world =
      glGetUniformLocation(gbuffer_shader, "normal_model_to_world");
  locations.diffuse_texture =
      glGetUniformLocation(gbuffer_shader, "diffuse_texture");
  locations.specular_texture =
      glGetUniformLocation(gbuffer_shader, "specular_texture");
  locations.normals_texture =
      glGetUniformLocation(gbuffer_shader, "normals_texture");
  locations.opacity_texture =
      glGetUniformLocation(gbuffer_shader, "opacity_texture");
  locations.has_diffuse_texture =
      glGetUniformLocation(gbuffer_shader, "has_diffuse_texture");
  locations.has_specular_texture =
      glGetUniformLocation(gbuffer_shader, "has_specular_texture");
  locations.has_normals_texture =
      glGetUniformLocation(gbuffer_shader, "has_normals_texture");
  locations.has_opacity_texture =
      glGetUniformLocation(gbuffer_shader, "has_opacity_texture");

  glUniformBlockBinding(gbuffer_shader, locations.ubo_CameraViewProjTransforms,
                        toU(UBO::CameraViewProjTransforms));
}

void fillShadowmapShaderLocations(GLuint shadowmap_shader,
                                  FillShadowmapShaderLocations &locations) {
  locations.ubo_LightViewProjTransforms =
      glGetUniformBlockIndex(shadowmap_shader, "LightViewProjTransforms");
  locations.light_index = glGetUniformLocation(shadowmap_shader, "light_index");
  locations.vertex_model_to_world =
      glGetUniformLocation(shadowmap_shader, "vertex_model_to_world");
  locations.opacity_texture =
      glGetUniformLocation(shadowmap_shader, "opacity_texture");
  locations.has_opacity_texture =
      glGetUniformLocation(shadowmap_shader, "has_opacity_texture");

  glUniformBlockBinding(shadowmap_shader, locations.ubo_LightViewProjTransforms,
                        toU(UBO::LightViewProjTransforms));
}

void fillAccumulateLightsShaderLocations(
    GLuint accumulate_lights_shader,
    AccumulateLightsShaderLocations &locations) {
  locations.ubo_CameraViewProjTransforms = glGetUniformBlockIndex(
      accumulate_lights_shader, "CameraViewProjTransforms");
  locations.ubo_LightViewProjTransforms = glGetUniformBlockIndex(
      accumulate_lights_shader, "LightViewProjTransforms");
  locations.light_index =
      glGetUniformLocation(accumulate_lights_shader, "light_index");
  locations.vertex_model_to_world =
      glGetUniformLocation(accumulate_lights_shader, "vertex_model_to_world");
  locations.vertex_world_to_clip =
      glGetUniformLocation(accumulate_lights_shader, "vertex_world_to_clip");
  locations.vertex_clip_to_world =
      glGetUniformLocation(accumulate_lights_shader, "vertex_clip_to_world");
  locations.depth_texture =
      glGetUniformLocation(accumulate_lights_shader, "depth_texture");
  locations.normal_texture =
      glGetUniformLocation(accumulate_lights_shader, "normal_texture");
  locations.shadow_texture =
      glGetUniformLocation(accumulate_lights_shader, "shadow_texture");
  locations.camera_position =
      glGetUniformLocation(accumulate_lights_shader, "camera_position");
  locations.inverse_screen_resolution = glGetUniformLocation(
      accumulate_lights_shader, "inverse_screen_resolution");
  locations.light_color =
      glGetUniformLocation(accumulate_lights_shader, "light_color");
  locations.light_position =
      glGetUniformLocation(accumulate_lights_shader, "light_position");
  locations.light_direction =
      glGetUniformLocation(accumulate_lights_shader, "light_direction");
  locations.light_intensity =
      glGetUniformLocation(accumulate_lights_shader, "light_intensity");
  locations.light_angle_falloff =
      glGetUniformLocation(accumulate_lights_shader, "light_angle_falloff");

  glUniformBlockBinding(accumulate_lights_shader,
                        locations.ubo_CameraViewProjTransforms,
                        toU(UBO::CameraViewProjTransforms));
  glUniformBlockBinding(accumulate_lights_shader,
                        locations.ubo_LightViewProjTransforms,
...
bonobo::mesh_data loadCone() {
  ...
  float vertexArrayData[65 * 3] = {
      ...
      0.38268f,  0.92388f,  -1.f, 0.f,       0.f,       0.f,
      0.70711f,  0.70711f,  -1.f, 0.f,       0.f,       0.f,
      0.92388f,  0.38268f,  -1.f, 0.f,       0.f,       0.f,
      ...
      -0.92388f, 0.38268f,  -1.f, 0.f,       0.f,       0.f,
      -0.70711f, 0.70711f,  -1.f, 0.f,       0.f,       0.f,
      -0.38268f, 0.92388f,  -1.f, 0.f,       1.f,       -1.f,
      0.f,       1.f,       -1.f, 0.38268f,  0.92388f,  -1.f,
      0.f,       1.f,       -1.f, 0.70711f,  0.70711f,  -1.f,
      0.f,       0.f,       -1.f, 0.92388f,  0.38268f,  -1.f,
      0.f,       0.f,       -1.f, 1.f,       0.f,       -1.f,
      0.f,       0.f,       -1.f, 0.92388f,  -0.38268f, -1.f,
      0.f,       0.f,       -1.f, 0.70711f,  -0.70711f, -1.f,
      0.f,       0.f,       -1.f, 0.38268f,  -0.92388f, -1.f,
