							  ...
							  unsigned int const vertical_split_count)
{
	...
	for (size_t i = 0; i < vert_count_x; i++)
	{
		for (size_t j = 0; j < vert_count_z; j++)
		{
			vertices[index] = glm::vec3(
				i * width / (vert_count_x-1), // float conversion ok?
				0,
				j * height / (vert_count_z-1));
			texCoords[index++] = glm::vec3(
				float(i) / float(vert_count_x-1),
				float(j) / float(vert_count_z-1),0.0f);
		}
	}

	auto trigs = std::vector<glm::uvec3>((vert_count_x - 1) * (vert_count_z - 1) * 2);

	for (int i = 0; i < vert_count - vert_count_z; i++) // hoppa över sista z raden, de ska inte binda till något
	{
	...
	glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::texcoords), 3, GL_FLOAT, GL_FALSE, 0, reinterpret_cast<GLvoid const *>(vertBufferSize));
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, /*! \todo how many bytes should the buffer contain? */ sizeof(glm::uvec3) * trigs.size(),
...
{
	auto calc_vertex = [radius](float st, float ct, float sp, float cp) -> glm::vec3
	{
		return radius * glm::vec3(st * sp, -cp, ct * sp);
	};
	auto calc_tangent = [radius](float st, float ct, float sp, float cp) -> glm::vec3
	{
		return glm::normalize(glm::vec3(ct, 0, -st));
	};
	auto calc_bitangent = [radius](float st, float ct, float sp, float cp) -> glm::vec3
	{
...
bonobo::mesh_data parametric_shapes::createTorus(float const major_radius, float const minor_radius, uint const major_split_count, uint const minor_split_count)
{
	...
	auto calc_vertex = [r1, r2](float st, float ct, float sp, float cp) -> glm::vec3
	{
		return glm::vec3((r1 + r2 * ct) * cp, -r2 * st, (r1 + r2 * ct) * sp);
	};
	auto calc_tangent = [r1](float st, float ct, float sp, float cp) -> glm::vec3
	{
		return glm::normalize(glm::vec3(-st * cp, -ct, -st * sp));
	};
	auto calc_bitangent = [r1, r2](float st, float ct, float sp, float cp) -> glm::vec3
	{
		return glm::normalize(glm::vec3(-(r1 + r2 * ct) * sp, 0, (r1 + r2 * ct) * cp));
	};
	return createShape(minor_split_count, major_split_count, calc_vertex, calc_tangent, calc_bitangent, glm::two_pi<float>(), glm::two_pi<float>());
}

bonobo::mesh_data parametric_shapes::createShape(unsigned int const longitude_split_count,
												 unsigned int const latitude_split_count,
												 std::function<glm::vec3(float st, float ct, float sp, float cp)> calc_vertex,
												 std::function<glm::vec3(float st, float ct, float sp, float cp)> calc_tangent,
												 std::function<glm::vec3(float st, float ct, float sp, float cp)> calc_bitangent,
												 float max_theta, float max_phi)
{
	...
	for (int i = 0; i < long_verts; i++)
	{
		...
		for (int j = 0; j < lat_verts; j++)
		{
			...
			// texCoords[idx] = glm::vec3(static_cast<float>(j) / (static_cast<float>(lat_verts)),
			//                              static_cast<float>(i) / (static_cast<float>(long_verts)),
			//                              0.0f);
	...
	for (int i = 0; i < vertices_nb - lat_verts; i++) //-lat_verts, hoppa över sista vertikala raden, de ska inte binda till något
	{
		// 0, 1, 2
		// 0, 2, 3
		if ((i + 1) % lat_verts == 0) // hoppa över alla punkter på nordpolen, de ska inte binda till något
			continue;
	...
	glGenBuffers(1, &data.bo);
	glBindBuffer(GL_ARRAY_BUFFER, data.bo);
	auto subBufferSize = sizeof(glm::vec3) * (vertices_nb); // whole buffer is x4 position, normal, binormal, tangent
	...
	glBufferSubData(GL_ARRAY_BUFFER, subBufferSize, subBufferSize, normals.data());
	glEnableVertexAttribArray(static_cast<unsigned int>(bonobo::shader_bindings::normals));
	glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::normals), 3, GL_FLOAT, GL_FALSE, 0, reinterpret_cast<GLvoid const *>(subBufferSize));

	glBufferSubData(GL_ARRAY_BUFFER, subBufferSize * 2, subBufferSize, bitangents.data());
	glEnableVertexAttribArray(static_cast<unsigned int>(bonobo::shader_bindings::binormals));
	glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::binormals), 3, GL_FLOAT, GL_FALSE, 0, reinterpret_cast<GLvoid const *>(subBufferSize * 2));

	glBufferSubData(GL_ARRAY_BUFFER, subBufferSize * 3, subBufferSize, tangents.data());
	glEnableVertexAttribArray(static_cast<unsigned int>(bonobo::shader_bindings::tangents));
	glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::tangents), 3, GL_FLOAT, GL_FALSE, 0, reinterpret_cast<GLvoid const *>(subBufferSize * 3));

	glBufferSubData(GL_ARRAY_BUFFER, subBufferSize * 4, subBufferSize, texCoords.data());
	glEnableVertexAttribArray(static_cast<unsigned int>(bonobo::shader_bindings::texcoords));
	glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::texcoords), 3, GL_FLOAT, GL_FALSE, 0, reinterpret_cast<GLvoid const *>(subBufferSize * 4));

	glBindBuffer(GL_ARRAY_BUFFER, 0u);
	// IBO
	glGenBuffers(1, &data.ibo);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, data.ibo);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(glm::uvec3) * trigs.size(), trigs.data(), GL_STATIC_DRAW);
