...
FPSCamera<T, P>::FPSCamera(T fovy, T aspect, T nnear, T nfar)
        : mWorld(), mMovementSpeed(1), mMouseSensitivity(1), mFov(fovy), mAspect(aspect), mNear(nnear), mFar(nfar),
          mProjection(), mProjectionInverse(), mMousePosition(glm::tvec2<T, P>(0.0f)) {
...
template<typename T, glm::precision P>
void FPSCamera<T, P>::Update(std::chrono::microseconds deltaTime, InputHandler &ih, bool ignoreKeyEvents,
                             bool ignoreMouseEvents, bool dragToMove) {
    ...
    if (!ih.IsMouseCapturedByUI() && !ignoreMouseEvents && (!dragToMove ||
                                                            ih.GetMouseState(GLFW_MOUSE_BUTTON_LEFT) & PRESSED)) {
    ...
    if (!ih.IsKeyboardCapturedByUI() && !ignoreKeyEvents) {
        ...
        T const movementModifier = ((ih.GetKeycodeState(GLFW_KEY_LEFT_CONTROL) & PRESSED)) ? 0.25f : ((
                ih.GetKeycodeState(GLFW_KEY_LEFT_SHIFT) & PRESSED)) ? 4.0f : 1.0f;
        auto const deltaTime_s = std::chrono::duration<T>(deltaTime);
        auto const movementChange =
                movementModifier * (mWorld.GetFront() * move + mWorld.GetRight() * strafe + mWorld.GetUp() * levitate);
