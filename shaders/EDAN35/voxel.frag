...
hit_t fvta_step(){
    ...
    vec3 last_index = floor((pos)/voxel_size);

    vec3 d = V;
    vec3 step = sign(d);

    vec3 next_position = (voxel_index + step) * voxel_size;
    vec3 tMax = (next_position - fract(start)) / d;
    vec3 tDelta = voxel_size / d * step;
    int max_steps = int(4.0/voxel_size);
    for (int i = 0; i < max_steps; i++){

        vec3 voxel_pos = (voxel_index + 0.5) * voxel_size;
        int material = int(round(texture(volume, voxel_pos).r*255));
        //if(isInside(voxel_pos) < 0.5) discard;
        if (voxel_pos.x > 1.0 || voxel_pos.y > 1.0 || voxel_pos.z > 1.0) discard;
        if (voxel_pos.x < 0.0 || voxel_pos.y < 0.0 || voxel_pos.z < 0.0) discard;
        if (material != 0){
            return hit_t(vec3(material/255.0), vec3(0), material);
        }
        //if(voxel_index == last_index) break;
        vec3 isSmallest = vec3(
            float(tMax.x < tMax.y)*float(tMax.x < tMax.z),
            float(tMax.y < tMax.z)*float(tMax.y < tMax.x),
            float(tMax.z < tMax.x)*float(tMax.z < tMax.y)
        );
        //if (!any(sign(isSmallest))) isSmallest.x = 1.0;

        voxel_index += step * isSmallest;

        tMax +=  isSmallest * tDelta;
        /*
                if(tMax.x < tMax.y){
                    if(tMax.x < tMax.z){
